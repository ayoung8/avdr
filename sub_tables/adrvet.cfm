...
<cfif Operation is "add">
  <cfstoredproc procedure="pkg_adr.ap_add_reaction_vet"  datasource="#application.avdrdsn#" username="#session.userid#" password="#session.password#">
  <cfprocparam type="in" dbvarname="reactionid" value="#ReactionID#" cfsqltype="cf_sql_integer">
  <cfprocparam type="in" dbvarname="vetid" value="#vet_id#" cfsqltype="cf_sql_integer">
  </cfstoredproc>
<cfElseIf Operation is "update">
  <cfstoredproc procedure="pkg_adr.ap_update_reaction_vet"  datasource="#application.avdrdsn#" username="#session.userid#" password="#session.password#">
  <cfprocparam type="in" dbvarname="reactionid" value="#ReactionID#" cfsqltype="cf_sql_integer">
  <cfprocparam type="in" dbvarname="vetid" value="#vet_id#" cfsqltype="cf_sql_integer">
  <cfprocparam type="in" dbvarname="oldvetid" value="#old_vet_id#" cfsqltype="cf_sql_integer">
  </cfstoredproc>
<cfelseif Operation is "Delete">
  <cfstoredproc procedure="pkg_adr.ap_delete_reaction_vet"  datasource="#application.avdrdsn#" username="#session.userid#" password="#session.password#">
  <cfprocparam type="in" dbvarname="reactionid" value="#ReactionID#" cfsqltype="cf_sql_integer">
  <cfprocparam type="in" dbvarname="vetid" value="#vet_id#" cfsqltype="cf_sql_integer">
  </cfstoredproc>
</cfif>
...
<form action="adrvet.cfm" name="detail" id="detail" method="post" target="">
<cfif session.adverse_role gt 0>
<table align="left" width="100%"  height = "20" border="0" cellspacing="0" cellspadding="0">
	<tr bgcolor="#408080">
	    <cfoutput>
  	    <td width="92%" style="font:10; color:F8CC30;"><b>#strLBLVet#</b></td>
		...
		<cfoutput query="rs">
			...
			<tr align="right"  valign="top" bgcolor=#txtColor# onMouseOver="this.bgColor='##408080'; document.all.delimg#row#.src='../images/delhl.bmp'; document.all.editimg#row#.src='../images/edithl.bmp';" onMouseOut="this.bgColor='#txtColor#'; document.all.delimg#row#.src='../images/del.bmp'; document.all.editimg#row#.src='../images/edit.bmp';">
				<td width="92%" align="left" style="font:10px">#rs.vet#</td>
				<td width="8%" align="center" style="font:13px">
				    <cfif session.adverse_role gt 1>
	 	  		    <A HREF="##" STYLE="color:navy;" onclick="SendToParent('#rs.vet_id#');"><img src="../Images/edit.bmp" name="editimg#row#" border="0"></a>&nbsp; 
	      			<A HREF="##" STYLE="color:navy;" onclick="var dodelete=window.confirm('#strAlertDeleteMe#'); if(dodelete) window.location='#SCRIPT_NAME#?Operation=Delete&reaction_id=#ReactionID#&vet_id=#rs.vet_id#';"><img src="../Images/del.bmp" name="delimg#row#" border="0"></A>
					</cfif>
				</td>
			</tr>
		</cfoutput>
	<tr>
		...
		<td><input type="hidden" name="vet_id"></td>
		<td><input type="hidden" name="old_vet_id"></td>
		</cfoutput>
	</tr>
</table>
</cfif>
...
<script language=javascript>
  parent.window.frmreaction.vet_id.value = '';
  
  function SendToParent(vetid) 
  {
    document.detail.Operation.value = "update";
    document.detail.old_vet_id.value = vetid;
	parent.window.frmreaction.vet_id.value = vetid;
  }
</script>
</body>
</html>